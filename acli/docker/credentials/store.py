...
def store(vault_name: VAULT_NAME_OPTION = DEFAULT_VAULT_NAME) -> None:
    ...
    if not existing:
        run_cmd(["op", "item", "create", "--format", "json", "--title", url,
                 "--category", "API Credential", "--vault", vault_name, "--"] + fields)
    else:
        run_cmd(["op", "item", "edit", "--format", "json", existing[0],
                 "--vault", vault_name, "--"] + fields)
